...
extension MovieDetailsView {
    ...
    var backdropView: some View {
        AsyncCachedImage(url: viewModel.movieDetails.backdropURL) { phase in
            switch phase {
            case .empty:
                ProgressView()
            ...
            case .failure(_):
                Image(systemName: "photo")
                    ...
                    .onAppear {
                        print("FAILED::::::::::\(viewModel.movieDetails.backdropURL)")
                    }
            @unknown default:
                ProgressView()
            }
    ...
    var posterView: some View {
        AsyncCachedImage(url: viewModel.movieDetails.posterURL) { phase in
            switch phase {
            case .empty:
                ProgressView()
            case .success(let image):
    ...
    var subInfoView: some View {
        VStack(alignment: .center) {
            HStack {
                RatingsView(rating: viewModel.movieDetails.voteAverage,
                            size: 60.0)
            ...
            VStack(alignment: .center, spacing: 10.0) {
                Text("Release Date: ").fontWeight(.heavy) + Text(DateHelper.getDisplayDate(str: viewModel.movieDetails.releaseDate))
                
                Text("Genre: ").fontWeight(.heavy) + Text(viewModel.getGenreDisplayText())
                
                Text("Duration: ").fontWeight(.heavy) + Text(viewModel.getMovieRuntimeText())
            }
            .font(.subheadline)
            .fontWeight(.medium)
            .multilineTextAlignment(.center)
...
#Preview {
    MovieDetailsView(viewModel: MovieDetailsViewModel(movieID: 872585))
}
